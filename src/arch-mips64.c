/* Copyright (C) 2006 CodeSourcery
   ...
   Adapted to MIPS64 by Lee Duhem <lee.duhem@gmail.com>, 2014
...
struct mips64_local_got_iterator {
...
/* Set up LGI to iterate over DSO's local GOT.  The caller should use
   mips64_get_local_got_entry to read the first entry.  */

static inline void
mips64_init_local_got_iterator (struct mips64_local_got_iterator *lgi, DSO *dso)
{
  lgi->dso = dso;
  lgi->entry_size = gelf_fsize (dso->elf, ELF_T_XWORD, 1, EV_CURRENT);
...
static inline int
mips64_get_local_got_entry (struct mips64_local_got_iterator *lgi)
{
  lgi->got_index++;
...
struct mips64_global_got_iterator {
  /* The DSO containing the GOT.  */
  DSO *dso;
...
/* Set up GGI to iterate over DSO's global GOT.  The caller should use
   mips64_get_global_got_entry to read the first entry.  */

static inline void
mips64_init_global_got_iterator (struct mips64_global_got_iterator *ggi, DSO *dso)
{
  GElf_Word sym_size;

  ggi->dso = dso;
  ggi->entry_size = gelf_fsize (dso->elf, ELF_T_XWORD, 1, EV_CURRENT);
...
static inline int
mips64_get_global_got_entry (struct mips64_global_got_iterator *ggi)
{
  ggi->sym_index++;
  ggi->got_addr += ggi->entry_size;
...
static int
mips64_arch_adjust (DSO *dso, GElf_Addr start, GElf_Addr adjust)
{
  struct mips64_local_got_iterator lgi;
  struct mips64_global_got_iterator ggi;
  GElf_Addr value;
  ...
  mips64_init_local_got_iterator (&lgi, dso);
  while (mips64_get_local_got_entry (&lgi))
    {
      value = buf_read_une64 (dso, lgi.got_entry);
      buf_write_ne64 (dso, lgi.got_entry, value + adjust);
    }
  ...
  mips64_init_global_got_iterator (&ggi, dso);
  while (mips64_get_global_got_entry (&ggi))
    {
      value = buf_read_une64 (dso, ggi.got_entry);
      if (ggi.sym.st_shndx != SHN_COMMON
	  && value >= start
	  && (value == ggi.sym.st_value
	      ? adjust_symbol_p (dso, &ggi.sym)
	      : ggi.sym.st_shndx != SHN_UNDEF))
	buf_write_ne64 (dso, ggi.got_entry, value + adjust);
    }
...
static int
mips64_adjust_dyn (DSO *dso, int n, GElf_Dyn *dyn, GElf_Addr start,
		   GElf_Addr adjust)
{
  switch (dyn->d_tag)
    {
...
/* Read the addend for a relocation in DSO.  If RELA is nonnull,
   use its r_addend, otherwise read a 64-bit in-place addend from
   address R_OFFSET.  */

static inline uint64_t
mips64_read_addend (DSO *dso, GElf_Addr r_offset, GElf_Rela *rela)
{
  return rela ? rela->r_addend : read_une64 (dso, r_offset);
}

/* Like mips64_read_addend, but change the addend to VALUE.  */

static inline void
mips64_write_addend (DSO *dso, GElf_Addr r_offset, GElf_Rela *rela,
		     uint64_t value)
{
  if (rela)
    rela->r_addend = (int64_t) value;
  else
...
static int
mips64_adjust_reloc (DSO *dso, GElf_Addr r_offset, GElf_Xword r_info,
		     GElf_Addr start, GElf_Addr adjust, GElf_Rela *rela)
{
  GElf_Addr value;
  GElf_Word r_sym;

  if (gelfx_r_type (dso->elf, r_info) == (R_MIPS_64 << 8 | R_MIPS_REL32))
    {
      r_sym = gelfx_r_sym (dso->elf, r_info);
      if (r_sym < dso->info_DT_MIPS_GOTSYM)
	{
	  ...
	  value = mips64_read_addend (dso, r_offset, rela);
	  mips64_write_addend (dso, r_offset, rela, value + adjust);
	}
    }
  return 0;
}
...
static int
mips64_adjust_rel (DSO *dso, GElf_Rel *rel, GElf_Addr start, GElf_Addr adjust)
{
  return mips64_adjust_reloc (dso, rel->r_offset, rel->r_info,
		  	      start, adjust, NULL);
}

static int
mips64_adjust_rela (DSO *dso, GElf_Rela *rela, GElf_Addr start, GElf_Addr adjust)
{
  return mips64_adjust_reloc (dso, rela->r_offset, rela->r_info,
		  	      start, adjust, rela);
}
...
static void
mips64_prelink_64bit_reloc (DSO *dso, GElf_Rela *rela, GElf_Addr value)
{
  assert (rela != NULL);
...
static int
mips64_prelink_reloc (struct prelink_info *info, GElf_Addr r_offset,
		      GElf_Xword r_info, GElf_Rela *rela)
{
  ...
  dso = info->dso;
  r_sym = gelfx_r_sym (dso->elf, r_info);
  r_type = gelfx_r_type (dso->elf, r_info);
  switch (r_type)
    {
    case R_MIPS_NONE:
      break;
    ...
    case R_MIPS_64 << 8 | R_MIPS_REL32:
	 ...
	 adjustment.  */
      if (rela != NULL || gelfx_r_sym (dso->elf, r_info) != 0)
	{
	  value = info->resolve (info, r_sym, r_type);
	  mips64_prelink_64bit_reloc (dso, rela, value);
	}
      break;
	...
	{
	  error (0, 0, "%s: R_MIPS_TLS_DTPMOD64 reloc in executable?",
		 dso->filename);
	  return 1;
	}
    ...
    case R_MIPS_TLS_DTPREL64:
      value = info->resolve (info, r_sym, r_type);
      mips64_prelink_64bit_reloc (dso, rela, value - TLS_DTV_OFFSET);
      break;
    ...
    default:
#ifdef DEBUG_MIPS
      error (0, 0, "(%s) %s: Unknown MIPS relocation type %x",
	     __func__, dso->filename, (uint32_t) gelfx_r_type (dso->elf, r_info));
#else
      error (0, 0, "%s: Unknown MIPS relocation type %x",
	     dso->filename, (uint32_t) gelfx_r_type (dso->elf, r_info));
#endif
      return 1;
    }
  return 0;
...
static int
mips64_prelink_rel (struct prelink_info *info, GElf_Rel *rel, GElf_Addr reladdr)
{
  DSO *dso;
  ...
  dso = info->dso;
  if (gelfx_r_type (dso->elf, rel->r_info) == (R_MIPS_64 << 8 | R_MIPS_REL32)
      && gelfx_r_sym (dso->elf, rel->r_info) >= dso->info_DT_MIPS_GOTSYM
      && read_une64 (dso, rel->r_offset) == 0)
    {
      rel->r_info = gelfx_r_info (dso->elf, gelfx_r_sym (dso->elf, rel->r_info),
				  R_MIPS_64 << 8 | R_MIPS_GLOB_DAT);
      write_ne64 (dso, rel->r_offset,
		  info->resolve (info, gelfx_r_sym (dso->elf, rel->r_info),
				 gelfx_r_type (dso->elf, rel->r_info)));
      return 2;
    }
  return mips64_prelink_reloc (info, rel->r_offset, rel->r_info, NULL);
}

static int
mips64_prelink_rela (struct prelink_info *info, GElf_Rela *rela,
		     GElf_Addr relaaddr)
{
  return mips64_prelink_reloc (info, rela->r_offset, rela->r_info, rela);
}
...
static int
mips64_get_tls (DSO *dso, struct prelink_conflict *conflict,
...
static int
mips64_prelink_conflict_reloc (DSO *dso, struct prelink_info *info,
			       ...
			       GElf_Rela *rela)
{
  ...
  conflict = prelink_conflict (info, gelfx_r_sym (dso->elf, r_info),
			       gelfx_r_type (dso->elf, r_info));
  if (conflict == NULL)
    {
      switch (gelfx_r_type (dso->elf, r_info))
	{
	case R_MIPS_TLS_DTPMOD64:
	case R_MIPS_TLS_TPREL64:
	  tls = info->curtls;
  ...
  switch (gelfx_r_type (dso->elf, r_info))
    ...
    case R_MIPS_TLS_DTPMOD64:
      if (conflict != NULL && mips64_get_tls (dso, conflict, &tls) == 1)
	return 1;
      value = tls->modid;
      break;

    case R_MIPS_TLS_DTPREL64:
      value += mips64_read_addend (dso, r_offset, rela) - TLS_DTV_OFFSET;
      break;
    ...
    default:
#ifdef DEBUG_MIPS
      error (0, 0, "(%s) %s: Unknown MIPS relocation type %x", dso->filename,
	     __func__, (uint32_t) gelfx_r_type (dso->elf, r_info));
#else
      error (0, 0, "%s: Unknown MIPS relocation type %x", dso->filename,
	     (uint32_t) gelfx_r_type (dso->elf, r_info));
#endif
      return 1;
    }
  ...
  if (entry == NULL)
    return 1;
  entry->r_addend = (int64_t) value;
  entry->r_offset = r_offset;
  entry->r_info = gelfx_r_info (dso->elf, 0, R_MIPS_64 << 8 | R_MIPS_REL32);
  return 0;
}

static int
mips64_prelink_conflict_rel (DSO *dso, struct prelink_info *info,
			     GElf_Rel *rel, GElf_Addr reladdr)
{
  return mips64_prelink_conflict_reloc (dso, info, rel->r_offset,
		  			rel->r_info, NULL);
}

static int
mips64_prelink_conflict_rela (DSO *dso, struct prelink_info *info,
			      GElf_Rela *rela, GElf_Addr relaaddr)
{
  return mips64_prelink_conflict_reloc (dso, info, rela->r_offset,
		  			rela->r_info, rela);
}

static int
mips64_arch_prelink_conflict (DSO *dso, struct prelink_info *info)
{
  struct mips64_global_got_iterator ggi;
  GElf_Addr value;
  struct prelink_conflict *conflict;
  ...
  while (mips64_get_global_got_entry (&ggi))
    {
      conflict = prelink_conflict (info, ggi.sym_index,
	...
	continue;
      if (buf_read_une64 (dso, ggi.got_entry) != value)
	{
	  entry = prelink_conflict_add_rela (info);
	  if (entry == NULL)
	    return 1;
...
static int
mips64_apply_conflict_rela (struct prelink_info *info, GElf_Rela *rela,
			    char *buf, GElf_Addr dest_addr)
{
  switch (gelfx_r_type (info->dso->elf, rela->r_info))
    {
    case R_MIPS_64 << 8 | R_MIPS_REL32:
...
/* BUF points to a 64-bit field in DSO that is subject to relocation.
   If the relocation is in a RELA section, RELA points to the relocation,
   otherwise it is null.  Add the addend to ADJUSTMENT and install the
   result.  */

static inline void
mips64_apply_adjustment (DSO *dso, GElf_Rela *rela, char *buf,
		         GElf_Addr adjustment)
{
  if (rela)
    adjustment += rela->r_addend;
  else
    adjustment += buf_read_une64 (dso, buf);
  buf_write_ne64 (dso, buf, adjustment);
}

static int
mips64_apply_reloc (struct prelink_info *info, GElf_Xword r_info,
		    GElf_Rela *rela, char *buf)
{
  ...
  switch (gelfx_r_type (dso->elf, r_info))
    ...
    case R_MIPS_64 << 8 | R_MIPS_REL32:
      mips64_apply_adjustment (dso, rela, buf,
		      	       info->resolve (info, gelfx_r_sym (dso->elf, r_info),
				       	      gelfx_r_type (dso->elf, r_info)));
      break;
...
static int
mips64_apply_rel (struct prelink_info *info, GElf_Rel *rel, char *buf)
{
  return mips64_apply_reloc (info, rel->r_info, NULL, buf);
}

static int
mips64_apply_rela (struct prelink_info *info, GElf_Rela *rela, char *buf)
{
  return mips64_apply_reloc (info, rela->r_info, rela, buf);
}

static int
mips64_rel_to_rela (DSO *dso, GElf_Rel *rel, GElf_Rela *rela)
{
  rela->r_offset = rel->r_offset;
  rela->r_info = rel->r_info;
  switch (gelfx_r_type (dso->elf, rel->r_info))
    {
    case R_MIPS_64 << 8 | R_MIPS_REL32:
    case R_MIPS_TLS_DTPREL64:
    case R_MIPS_TLS_TPREL64:
      /* These relocations have an in-place addend.  */
      rela->r_addend = (int64_t) read_une64 (dso, rel->r_offset);
      break;
...
#ifdef DEBUG_MIPS
      error (0, 0, "(%s) %s: Unknown MIPS relocation type %x", dso->filename,
	     __func__, (uint32_t) gelfx_r_type (dso->elf, rel->r_info));
#else
      error (0, 0, "%s: Unknown MIPS relocation type %x", dso->filename,
	     (uint32_t) gelfx_r_type (dso->elf, rel->r_info));
#endif
      return 1;
    }
  return 0;
...
static int
mips64_rela_to_rel (DSO *dso, GElf_Rela *rela, GElf_Rel *rel)
{
  rel->r_offset = rela->r_offset;
  rel->r_info = rela->r_info;
  switch (gelfx_r_type (dso->elf, rela->r_info))
    {
    case R_MIPS_NONE:
      break;
...
#ifdef DEBUG_MIPS
      error (0, 0, "(%s) %s: Unknown MIPS relocation type %x", dso->filename,
	     __func__, (uint32_t) gelfx_r_type (dso->elf, rela->r_info));
#else
      error (0, 0, "%s: Unknown MIPS relocation type %x", dso->filename,
	     (uint32_t) gelfx_r_type (dso->elf, rela->r_info));
#endif
      return 1;
    }
  return 0;
...
static int
mips64_need_rel_to_rela (DSO *dso, int first, int last)
{
  Elf_Data *data;
  Elf_Scn *scn;
  GElf_Rel rel;
	  ...
	  for (ndx = 0; ndx < maxndx; ndx++)
	    {
		  ...
		  case R_MIPS_64 << 8 | R_MIPS_REL32:
		    ...
		    if (gelfx_r_sym (dso->elf, rel.r_info) != 0
		        && (gelfx_r_sym (dso->elf, rel.r_info) < dso->info_DT_MIPS_GOTSYM
			|| read_une64 (dso, rel.r_offset) != 0))
		      return 1;
		    break;
...
#ifdef DEBUG_MIPS
		    error (0, 0, "(%s) %s: Unknown MIPS relocation type %x",
			   __func__, dso->filename, (uint32_t) gelfx_r_type (dso->elf, rel.r_info));
#else
		    error (0, 0, "%s: Unknown MIPS relocation type %x",
			   dso->filename, (uint32_t) gelfx_r_type (dso->elf, rel.r_info));
#endif
		    return 1;
		}
	    }
...
static int
mips64_reloc_size (int reloc_type)
{
  return 8;
}

static int
mips64_reloc_class (int reloc_type)
{
  switch (reloc_type)
    {
    case R_MIPS_TLS_DTPMOD64:
...
static int
mips64_arch_prelink (struct prelink_info *info)
{
  struct mips64_global_got_iterator ggi;
  DSO *dso;
  GElf_Addr value;
  ...
  while (mips64_get_global_got_entry (&ggi))
    {
      value = info->resolve (info, ggi.sym_index,
		      	     R_MIPS_64 << 8 | R_MIPS_REL32);
      if (ggi.sym.st_shndx == SHN_UNDEF
	  || ggi.sym.st_shndx == SHN_COMMON)
	buf_write_ne64 (dso, ggi.got_entry, value);
      else
	{
...
static int
mips64_arch_undo_prelink (DSO *dso)
{
  struct mips64_global_got_iterator ggi;
  ...
  while (mips64_get_global_got_entry (&ggi))
    if (ggi.sym.st_shndx == SHN_UNDEF)
      ...
      buf_write_ne64 (dso, ggi.got_entry, ggi.sym.st_value);
    else if (ggi.sym.st_shndx == SHN_COMMON)
      /* Type D in the table above.  */
      buf_write_ne64 (dso, ggi.got_entry, 0);
  return ggi.failed;
}

static int
mips64_undo_prelink_rel (DSO *dso, GElf_Rel *rel, GElf_Addr reladdr)
{
  ...
  if (gelfx_r_type (dso->elf, rel->r_info) == (R_MIPS_64 << 8 | R_MIPS_GLOB_DAT))
    {
...
PL_ARCH = {
  ...
  .machine = EM_MIPS,
  .max_reloc_size = 8,
  .dynamic_linker = "/lib64/ld.so.1",
  /* MIPS does not use COPY relocs or jump slots.  Pick a value outside
     the ELF64_R_TYPE range.  */
  .R_COPY = ~0U,
  .R_JMP_SLOT = ~0U,
  /* R_MIPS_REL32 relocations against symbol 0 do act as relative relocs,
     but those against other symbols don't.  */
  .R_RELATIVE = ~0U,
  /* ld.so can give some conflicts that have reloc_class 0. No idea what
     is that. */
  .rtype_class_valid = RTYPE_CLASS_PLT,
  .arch_adjust = mips64_arch_adjust,
  .adjust_dyn = mips64_adjust_dyn,
  .adjust_rel = mips64_adjust_rel,
  .adjust_rela = mips64_adjust_rela,
  .prelink_rel = mips64_prelink_rel,
  .prelink_rela = mips64_prelink_rela,
  .prelink_conflict_rel = mips64_prelink_conflict_rel,
  .prelink_conflict_rela = mips64_prelink_conflict_rela,
  .arch_prelink_conflict = mips64_arch_prelink_conflict,
  .apply_conflict_rela = mips64_apply_conflict_rela,
  .apply_rel = mips64_apply_rel,
  .apply_rela = mips64_apply_rela,
  .rel_to_rela = mips64_rel_to_rela,
  .rela_to_rel = mips64_rela_to_rel,
  .need_rel_to_rela = mips64_need_rel_to_rela,
  .reloc_size = mips64_reloc_size,
  .reloc_class = mips64_reloc_class,
  .arch_prelink = mips64_arch_prelink,
  .arch_undo_prelink = mips64_arch_undo_prelink,
  .undo_prelink_rel = mips64_undo_prelink_rel,
  /* Although TASK_UNMAPPED_BASE is 0x5555555555, we leave some
  ...
  .mmap_base = 0x5c00000000UL,
  .mmap_end =  0xFFFF9F0000UL,
